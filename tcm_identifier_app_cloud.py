# -*- coding: utf-8 -*-
"""
中药化合物智能鉴定平台 - 云端部署增强版 v5.16（完整修复版）
======================================================================
基于 v5.15 修复以下漏洞：
1. 候选排序使用综合得分（而非仅碎片数）
2. 碎片匹配引入覆盖率惩罚（匹配数/总实验碎片数）
3. 碎片匹配选择最佳匹配（最小误差）
4. 诊断离子分类优先使用化合物类型字段
5. 无索引碎片不再自动关联全部文献（避免虚高）
6. 实现真正的并行处理（ThreadPoolExecutor）
"""
...
class UltimateGardeniaIdentifier:
        ...
        for key, group in df.groupby('_group_key', sort=False):
            ...
            if '中性丢失' in group.columns:
                for loss_str in group['中性丢失'].dropna():
                    for loss in re.split(r'[;；,，]+', str(loss_str)):
                        loss = loss.strip()
                        if loss:
                            try:
                                neutral_losses_set.add(float(loss))
                            except:
                                pass
            merged_neutral_loss = ','.join(str(l) for l in sorted(neutral_losses_set)) if neutral_losses_set else ''

            # ----- 处理正离子数据 -----
            pos_mz_frag_map = {}
            for _, row in group.iterrows():
                mz_pos = row.get('准分子离子（正）')
                    ...
                    try:
                        ...
                        if pd.notna(frag_str) and str(frag_str).strip():
                            ...
                            if pd.notna(src_val) and str(src_val).strip():
                                for s in re.split(r'[;；,，]+', str(src_val)):
                                    s = s.strip()
                                    if s:
            ...
            for mz_val, frag_items in pos_mz_frag_map.items():
                ...
                for frag_mz, indices in sorted(frag_dict.items()):
                    if indices:
                        ...
                        new_frag_parts.append(f"{frag_mz:.3f}:{idx_str}")
                    else:
                ...
                new_row = {
                    ...
                    '化合物类型': compound_type,
                    '准分子离子（正）': mz_val,
                    '碎片离子（正）': merged_frag_pos,
                    '准分子离子（负）': '',
                    '碎片离子（负）': '',
                    '文献来源': merged_sources_str,
                    '保留时间': rt_val,
                    '加合物（正）': adduct_pos,
                    '加合物（负）': '',
                    '中性丢失': merged_neutral_loss,
                }
                        ...
                        if pd.notna(frag_str) and str(frag_str).strip():
                            ...
                            if pd.notna(src_val) and str(src_val).strip():
                                for s in re.split(r'[;；,，]+', str(src_val)):
                                    ...
                                    if s:
                                        row_sources.append(s)
                            neg_mz_frag_map.setdefault(mz_val, []).append((str(frag_str), row_sources))
                    except (ValueError, TypeError):
                        pass

            for mz_val, frag_items in neg_mz_frag_map.items():
                frag_dict = {}
                for frag_str, row_sources in frag_items:
                    parts = re.split(r'[、;，,\s]+', frag_str)
                    for part in parts:
                ...
                for frag_mz, indices in sorted(frag_dict.items()):
                    if indices:
                        ...
                        new_frag_parts.append(f"{frag_mz:.3f}")
                merged_frag_neg = '; '.join(new_frag_parts) if new_frag_parts else ''
    ...
    def _build_optimized_index(self):
        ...
        for idx, row in self.database.iterrows():
            ...
            if '保留时间' in row and pd.notna(row['保留时间']):
                try:
                    self.rt_values[idx] = float(row['保留时间'])
                except:
                    pass
    ...
    def identify_compound(self, precursor, return_top=1, score_threshold=0):
        ...
        for idx in candidate_range:
            ...
            base_score = self._calculate_base_score(
                lvl_id, ppm, len(matched_fragments), len(diagnostic_ions),
                diag_weights, neutral_loss_matches, rt_deviation,
                matched_lit_count, coverage
            )
            ...
            candidate = {
                ...
                # 保存原始母离子信息供报告使用
                'precursor_rt': precursor['retention_time'],
                'precursor_fragments_list': precursor['fragments'].tolist(),
                'precursor_total_fragments': total_exp_fragments
            }
    ...
    def generate_report(self, herb_name=None):
        ...
        for cand in valid_candidates:
            ...
            matched_frags = cand['matched_fragments']
            diag_ions = cand['diagnostic_ions']
            diag_weights = cand['diag_weights']

            matched_details = cand.get('matched_details', [])
            frag_lit_map_str = ''
            if matched_details:
                unique_map = {}
            ...
            if cas and cas != 'nan' and cas != '':
                ...
                merge_key = f"{name_cn}_{formula}"

            record = {
                '序号': 0,
                '出峰时间t/min': cand['precursor_rt'],
                '化合物中文名': name_cn,
                '化合物英文名': en_name,
                '分子式': formula,
                'CAS号': cand['cas'],
                ...
                '评级名称': cand['lvl_name'],
                '置信度': cand['confidence'],
                '报告建议': cand['suggestion'],
                '基础得分': cand['base_score'],
                '综合得分': cand['base_score'],
            ...
            herbs_collection[merge_key].add(cand['herb'])
...
st.set_page_config(
    page_title="中药化合物智能鉴定平台 v5.16（完整修复版）",
    page_icon="🌿",
    layout="wide",
